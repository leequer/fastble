...
class DeviceAdapter(private val context: Context) : BaseAdapter() {
    ...
    fun removeDevice(bleDevice: BleDevice) {
        bleDeviceList.removeAll { it.key == bleDevice.key }
    }

    fun clearConnectedDevice() {
        bleDeviceList.removeAll { BleManager.getInstance().isConnected(it) }
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        holder.btn_connect.setOnClickListener { mListener?.onConnect(bleDevice!!) }
        holder.btn_disconnect.setOnClickListener { mListener?.onDisConnect(bleDevice!!) }
        holder.btn_detail.setOnClickListener { mListener?.onDetail(bleDevice!!) }
